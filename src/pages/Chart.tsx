...
export default function Chart() {
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      crosshair: {
        vertLine: { color: '#2a3a4a', labelBackgroundColor: '#1a2535' },
        horzLine: { color: '#2a3a4a', labelBackgroundColor: '#1a2535' },
      },
      rightPriceScale: {
        borderVisible: false,
        textColor: '#3a4d5e',
  ...
  return (
    <div style={{ height: '100vh', background: '#0a0b0d', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* TF bar */}
      <div style={{ display: 'flex', alignItems: 'center', padding: '5px 10px 6px', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', background: '#0a0b0d', flexShrink: 0, position: 'relative' }}>
        ...
        <div style={{ display: 'flex', flex: 1, overflowX: 'auto', gap: 0 }}>
          {ALL_TF.filter(t => visibleTF.includes(t.label)).map((t) => (
            <button key={t.label} onClick={() => setActiveTF(t.api)}
              style={{ padding: '5px 9px', fontSize: 12, fontWeight: 600, color: activeTF === t.api ? '#fff' : 'var(--mu)', background: activeTF === t.api ? '#1a2535' : 'none', border: 'none', borderRadius: 8, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {/* Bouton + */}
        <button onClick={() => setShowTFMenu(!showTFMenu)}
          style={{ background: 'none', border: 'none', color: '#fff', fontSize: 18, fontWeight: 700, cursor: 'pointer', padding: '0 6px', flexShrink: 0 }}>
          +
        </button>
  ...
  <>
    ...
    <div style={{ position: 'absolute', bottom: 'calc(100% + 6px)', right: 8, width: 120, maxHeight: 280, overflowY: 'auto', background: '#1c1f24', border: '1px solid var(--border)', borderRadius: 14, zIndex: 50, boxShadow: '0 -8px 30px rgba(0,0,0,.7)' }}>
      {ALL_TF.map((t) => {
        ...
        return (
          <div key={t.label}
            ...
            style={{ padding: '8px 12px', fontSize: 12, fontWeight: isActive ? 700 : 500, color: isActive ? 'var(--g)' : '#fff', borderBottom: '1px solid #23262b', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span>{t.label}</span>
            <span
              onClick={(e) => {
                e.stopPropagation()
      ...
      {/* Exec zone */}
      <div style={{ padding: '9px 12px 18px', background: '#0a0b0d', flexShrink: 0, position: 'relative' }}>

        {/* Order type dropdown */}
        {showOTMenu && (
          <>
            <div onClick={() => setShowOTMenu(false)} style={{ position: 'fixed', inset: 0, zIndex: 40 }} />
            <div style={{ position: 'absolute', bottom: 'calc(100% + 8px)', left: 12, width: 200, background: '#1c1f24', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden', zIndex: 50, boxShadow: '0 -12px 40px rgba(0,0,0,.7)' }}>
              {ORDER_TYPES.map((ot) => (
                <div key={ot} onClick={() => { setOrderType(ot); setShowOTMenu(false) }}
                  style={{ padding: '12px 16px', fontSize: 14, fontWeight: orderType === ot ? 700 : 500, color: orderType === ot ? 'var(--g)' : '#fff', borderBottom: '1px solid #23262b', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ width: 40, height: 40, borderRadius: '50%', border: '2px solid rgba(255,255,255,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <select value={lot} onChange={(e) => setLot(e.target.value)}
              ...
              {LOT_OPTIONS.map((l) => <option key={l} value={l} style={{ background: '#1c1f24' }}>{l}</option>)}
            </select>
          </div>
